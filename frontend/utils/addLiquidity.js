...
export const addLiquidity = async (
  ...
  try {
    ...
    tx = await exchangeContract.addLiquidity(addCDAmountWei.toString(), {
      value: addEtherAmountWei.toString(),
    });
    await tx.wait();
  } catch (err) {
    console.log(err);
...
export const calculateCD = async (addEther = "0", etherBalanceContract, cdTokenReserve) => {
  ...
  const _addEtherAmountWei = ethers.utils.parseEther(addEther);
